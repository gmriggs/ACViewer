...
{
    public class LocalDatabaseCache : LocalDatabase
    {
        private readonly ConcurrentDictionary<uint, IDBObj> _cache = new ConcurrentDictionary<uint, IDBObj>();

        public LocalDatabaseCache(Action<DatDatabaseOptions> options, IDatBlockAllocator blockAllocator = null) : base(options, blockAllocator) { }

        public LocalDatabaseCache(string datFilePath, DatAccessType accessType = DatAccessType.Read) : base(datFilePath, accessType) { }
