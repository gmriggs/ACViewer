...
namespace ACE.DatLoader
{
    static class UnpackableExtensions
    {
        ...
        public static void UnpackPackedHashTable<T>(this Dictionary<SpellId, T> value, BinaryReader reader) where T : IUnpackable, new()
        {
            var totalObjects = reader.ReadUInt16();
            /*var bucketSize = */
            reader.ReadUInt16();
            ...
            for (int i = 0; i < totalObjects; i++)
            {
                ...
                var item = new T();
                item.Unpack(reader);
                value.Add((SpellId)key, item);
            }
        }

        public static void UnpackPackedHashTable<T>(this Dictionary<EquipmentSet, T> value, BinaryReader reader) where T : IUnpackable, new()
        {
            var totalObjects = reader.ReadUInt16();
            /*var bucketSize = */
            reader.ReadUInt16();
            ...
            for (int i = 0; i < totalObjects; i++)
            {
                ...
                var item = new T();
                item.Unpack(reader);
                value.Add((EquipmentSet)key, item);
            }
        }
        ...
        /// A list that uses a Int32 for the length.
        /// </summary>
        public static void Unpack(this List<SpellId> value, BinaryReader reader)
        {
            var totalObjects = reader.ReadInt32();
