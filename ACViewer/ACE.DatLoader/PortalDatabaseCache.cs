...
{
    public class PortalDatabaseCache : PortalDatabase
    {
        private readonly ConcurrentDictionary<uint, IDBObj> _cache = new ConcurrentDictionary<uint, IDBObj>();

        public PortalDatabaseCache(Action<DatDatabaseOptions> options, IDatBlockAllocator blockAllocator = null) : base(options, blockAllocator) { }

        public PortalDatabaseCache(string datFilePath, DatAccessType accessType = DatAccessType.Read) : base(datFilePath, accessType) { }
